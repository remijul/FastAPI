...
### Solution for Task 6: Additional validation rules

Add these validation rules to the `IrisFeatures` model:
...
def check_realistic_values(cls, values):
    ...
    if all(key in values for key in ['sepal_length', 'sepal_width', 'petal_length', 'petal_width']):
        ...
        # Iris-specific checks based on known species characteristics
        # Setosa typically has small petals
        if petal_length < 2 and petal_width < 0.7:
            # For Setosa-like samples, check that sepals are in the right range
            if sepal_width < 2 or sepal_width > 4.5:
                raise ValueError("For flowers with small petals (likely Setosa), sepal width is typically between 2-4.5cm")
        
        # Versicolor and Virginica have larger petals
        if petal_length > 3:
            # For these species, check petal width to length ratio
            if petal_width / petal_length > 0.7:
                raise ValueError("For flowers with longer petals, the petal width to length ratio is typically less than 0.7")
                
        return values
```

### Solution for Task 7: Logging system

Create a new file `app/utils/logging.py`:
...
def setup_logging(log_dir: str = "logs", level: int = logging.INFO) -> logging.Logger:
    ...
    # Create a special handler for request logs
...
def log_request(logger: logging.Logger, method: str, path: str, status_code: int, client: str, duration_ms: float):
    """Log an API request"""
    logger.info(f"REQUEST - Method: {method} - Path: {path} - Status: {status_code} - Client: {client} - Duration: {duration_ms:.2f}ms")

def log_model_prediction(logger: logging.Logger, model_name: str, input_features, prediction):
    """Log a model prediction"""
    logger.info(f"PREDICTION - Model: {model_name} - Input: {input_features} - Prediction: {prediction}")
...
Update `app/main.py` to use the new logging system:

```python
from app.utils.logging import setup_logger, log_request, log_error
...
async def log_requests(request: Request, call_next):
    ...
    # Log the request
    log_request(logger, method, path, response.status_code, client_host, process_time)
...
In the next step, we'll learn about API documentation and testing, which are essential for ensuring the quality and usability of your API.